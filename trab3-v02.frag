...
vec4 calculanormal (vec3 pontoA,vec3 pontoB, vec3 pontoC,vec3 pontoD){
...
vec3 verificaNormal(vec4 normalface1,vec4 normalface2,vec4 normalface3,vec4 normalface4,vec4 normalface5,vec4 normalface6,vec3 ponto){
...
int verificaFace(vec4 normalface1,vec4 normalface2,vec4 normalface3,vec4 normalface4,vec4 normalface5,vec4 normalface6,vec3 ponto){
...
Cuboid inicializaCubo (Cuboid cubo){
	...
	vec4 normalface1 = calculanormal(cubo.v1,cubo.v2,cubo.v3,cubo.v4);
	vec4 normalface2 = calculanormal(cubo.v2,cubo.v4,cubo.v6,cubo.v8); 
	vec4 normalface3 = calculanormal(cubo.v5,cubo.v6,cubo.v7,cubo.v8); 
	vec4 normalface4 = calculanormal(cubo.v1,cubo.v3,cubo.v5,cubo.v7); 
	vec4 normalface5 = calculanormal(cubo.v1,cubo.v2,cubo.v5,cubo.v6); 
	vec4 normalface6 = calculanormal(cubo.v3,cubo.v4,cubo.v7,cubo.v8);  
	...
	vec3 p = getP(start); //ponto de entrada
		
	cubo.o.normalIn =  verificaNormal(normalface1,normalface2,normalface3,normalface4,normalface5,normalface6,p);
	//verifica normal da face que o raio sai
	vec3 s=camPos+final*camDir; //ponto de saida
	cubo.o.normalOut = verificaNormal(normalface1,normalface2,normalface3,normalface4,normalface5,normalface6,s);
 	
	if(p.y==cubo.v8.y)
		cubo.o.cor = vec3(1.0,0.0,0.0);
...
void main(void){
Cuboid cubo1;
	cubo1.v1=vec3(-0.1,-0.1,-0.1);
	cubo1.v8=vec3(0.1,0.1,0.1);
	
	cubo1.o.cor=vec3(0.0,0.0,1.0);
	cubo1 = inicializaCubo(cubo1);

Cuboid cubo2;
	cubo2.v1=vec3(0.0,0.0,0.0);
	cubo2.v8=vec3(1.0,1.0,1.0);
	
	cubo2.o.cor=vec3(0.0,0.0,1.0);
	cubo2 = inicializaCubo(cubo2);
	...
	Objeto obj_1=uniao(cubo1.o,cubo1.o,false);
	obj_1=diferenca(cubo1.o,cubo2.o,true);
//	Objeto obj_1=intersecao(esfera1.o,cubo1.o,true);

	//obj_1 = uniao(obj_1,esfera3.o,false);

	//obj_1 = diferenca(obj_1,esfera4.o,false);

	//obj_1 = uniao(obj_1,cubo1.o,true);

	//Objeto objNovo=cubo1.o;
	//pinta(objNovo.normalIn,objNovo.cor,camPos+objNovo.entra*camDir);
}
